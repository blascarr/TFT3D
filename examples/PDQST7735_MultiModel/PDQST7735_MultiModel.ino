...
mesh model_1( &nodelist, &facelist );
mesh model_2( &nodelist, &facelist );
...
void setup() {
  ...
  model_1.setnodes ( &nodelist );
  model_1.setfaces ( &facelist );

  model_2.setnodes ( &nodelist );
  model_2.setfaces ( &facelist );


  model_1.setdraw_type(1);
  model_1.setskip_tick(40);
  model_1.setframe_skip(50);

  model_2.setdraw_type(1);
  model_2.setskip_tick(40);
  model_2.setframe_skip(50);
  //define colors for background
  int c[6] ={BLACK, GREEN, GREEN, BLACK, GREEN, YELLOW};
  model_1.setColors( c );
  model_1.set_tftsize( tftdisplay.width(), tftdisplay.height() );
  model_1.set_tftpos( tftdisplay.width()/2, tftdisplay.height()/4 );

  model_2.setColors( c );
  model_2.set_tftsize( tftdisplay.width(), tftdisplay.height() );
  model_2.set_tftpos(  tftdisplay.width()/2, 3*tftdisplay.height()/4 );
}

void loop() {
  model_1.update( abs(r1%360 ), abs(r1%360 ),0 );
  model_2.update( abs(r2%360 ), abs(r2%360 ),0 );
  r1++;
  r2--;
  model_1.draw( &tft );
  model_2.draw( &tft );
