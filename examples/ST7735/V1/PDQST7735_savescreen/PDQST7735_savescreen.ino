...
void loop() {
  ...
  if( millis() - auxtimer > 80){
    auxtimer = millis();
    xpos+=xdir;
    ypos+=ydir;
    if(xpos > tftdisplay.width() - xmargin ){  xdir = -xinc; }
    if(xpos < 0 + xmargin ){  xdir = xinc; }
    if(ypos > tftdisplay.height() - ymargin ){  ydir = -yinc; }
    if(ypos < 0 + ymargin ){  ydir = yinc; }
    model.set_tftpos( xpos, ypos );
  };
